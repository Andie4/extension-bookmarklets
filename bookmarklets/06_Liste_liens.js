...
function listLinks(){
  ...
  Array.from(links).forEach(function (link) {
    ...
    if (link.getAttribute('aria-label')) {
      ...
      if (tc.trim()!=='') {
        notes+=' Check that the accessible name does not contradict the text on screen<br>';
      }
      warn=true;
      if (accName.trim().toLowerCase().indexOf(tc.trim().toLowerCase()) === -1) {
        notes+='- On-screen text does not appear in <code>aria-label</code>. Looks like a <a href="https://www.w3.org/WAI/WCAG21/Understanding/label-in-name.html" target="_blank">2.5.3 Label In Name</a> failure<br>';
      }
    }
    ...
    if (link.getAttribute('aria-labelledby')) {
      ...
      if (sources.length > 1) {
        ...
        notes+='- Link gets accessible name from <code>aria-labelledby</code> (multiple sources). Check that the accessible name does not contradict the text on screen<br>';
        warn=true;
      } else {
        accName=document.querySelector('#' + link.getAttribute('aria-labelledby')).textContent;
        notes+='- Link gets accessible name from <code>aria-labelledby</code> (single source). Check that the accessible name does not contradict the text on screen<br>';
        warn=true;
      }
      if (accName.trim().toLowerCase().indexOf(tc.trim().toLowerCase()) === -1) {
        notes+='- On-screen text does not appear in <code>aria-labelledby sources</code>. Looks like a <a href="https://www.w3.org/WAI/WCAG21/Understanding/label-in-name.html" target="_blank">2.5.3 Label In Name</a> failure<br>';
      }
    }
    ...
    if (isHidden(link)) {
      ...
      if (isHidden(link)) {
        ...
        notes+='Link *was* hidden but has been temporarily revealed on the page<br>';
      }
    }
    ...
    if (link.getAttribute('role') === 'button') {
      notes+='- Link has a <code>role</code> of <code>button</code>. Check that it behaves like a <code>button</code> and is not used as navigation.<br>';
      warn=true;
    }
  
    if (link.getAttribute('title') && link.getAttribute('title') !== tc) {
      notes+='- Link has a <code>title</code> which is different from text content. This <code>title</code> content -- "' +
    ...
    if (link.getAttribute('title') && link.getAttribute('title') === tc) {
      notes+='Link has a <code>title</code> which is the same as the text content and therefore adds no extra useful information/context. This <code>title</code> content -- "' +
        link.getAttribute('title') +
        '" -- will not be perceivable to assistive tech, keyboard and touch screen users<br>';
    }
  
    if (link.tagName === 'A') {
      if ((link.getAttribute('href') === null) && !link.getAttribute('tabindex') && link.getAttribute(
          'tabindex') !== '-1') {
        notes+='- Link does not have an <code>href</code>, is not keyboard-focusable<br>';
        warn=true;
      }
  
      if (((link.getAttribute('href') !== null) && (!link.getAttribute('href'))) && !link.getAttribute(
          'tabindex') && link.getAttribute('tabindex') !== '-1') {
        notes+='- Link does have an <code>href</code> but it has no value, so it is keyboard-focusable<br>';
        warn=true;
      }
  
      if (!link.getAttribute('href') && link.getAttribute('tabindex') && link.getAttribute('tabindex') !==
        '-1') {
        notes+='- Link does not have an <code>href</code>, but is focusable because it has been provided with a positive <code>tabindex</code><br>';
        warn=true;
      }
  
      if (link.getAttribute('tabindex') && ((link.getAttribute('tabindex') !==
        '-1')&&(link.getAttribute('tabindex') !==
        '0'))) {
        notes+='- Link has a positive <code>tabindex</code> (not -1 or 0). Could cause a <a href="https://www.w3.org/TR/WCAG21/#focus-order" target="_blank">2.4.3 Focus order</a> failure.<br>';
        warn=true;
      }
  
      if (link.getAttribute('href') && link.getAttribute('tabindex') && link.getAttribute('tabindex') !==
        '-1') {
        notes+='Link has a <code>tabindex</code> but it is not needed because the <code>href</code> makes it focusable<br>';
      }
  
      if (link.getAttribute('role') === 'link') {
        notes+='Link has a <code>role</code> of <code>link</code>. Not needed as it is an <code>a</code> element that is a link by default<br>';
      }
    }
  
    if (link.tagName !== 'A' && link.getAttribute('role') === 'link') {
      notes+='- This has a <code>role</code> of <code>link</code> but is not an <code>a</code> element. Check that it is keyboard-operable (should activate with <kbd>Enter</kbd> key)<br>';
      warn=true;
    }
    ...
    row +='</td>';
    if ((accName.trim() !== tc.trim()) && (accName.trim().toLowerCase() === tc.trim()
        .toLowerCase())) {
      notes+='- Same text but case difference noted (likely not an issue)';
    }
    row +='<td>';
    if (warn) {
      row+='<div class="issues">Possible issue(s) found with this link</div>';
    }
    if (err) {
      row+='<div class="issues">Definite issue(s) found with this link</div>';
    }
    consoleOutput='Link \'' + tc.trim() + '\':\n'+notes+'Markup with issue:\n'+snippet+'\n---------------\n';
    row +=notes + '<a href="' + href + '" target="_blank" aria-label="' + accName +
      '">🔗</a> <label for="l' + i + '">Links to:</label><input id="l' + i + '" class="linkToCopy" type="text" value="' + href + '"> <button class="showSnippet" type="button" aria-label="Show markup snippet" aria-expanded="false"><code>&lt;/&gt;</code></button><div class="snippet" hidden><label for="snip'+ i + '">Markup snippet</label><textarea id="snip'+ i + '" aria-label="Markup snippet for this node">' + snippet + '</textarea><button type="button" class="decrapulate" aria-label="De-crapulate this markup snippet">De-crapulate</button></div></td>';
    row +='<td><button data-link-ref="' + i + '" class="highlightButton" type="button" aria-pressed="false" aria-label="Highlight this issue on the page visually">Show</button></td>';
    row +='</tr>';
    i++;
    if (warn || err) {
      issueCount++;
  ...
  s+='var linksToCopy=document.querySelectorAll(".linkToCopy");Array.from(linksToCopy).forEach(linkToCopy => {linkToCopy.addEventListener("focus", e => {linkToCopy.select();});});';
  ...
  s+='}window.addEventListener("load", (event) => {showLinks();});</script>';
  s+='<script>';
