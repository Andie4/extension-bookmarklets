javascript: (function() {
    ...
    function listThingsWithTitles() {
        ...
        Array.from(elsWithTitles).forEach(function(elWithTitle) {
            ...
            row += '<td>';
            if (warn) {
                row += '<div class="issues">Possible title issue found with this element</div>';
            }
            if (err) {
                row += '<div class="issues">Definite title issue found with this element</div>';
            }
            consoleOutput = 'Element with title \'' + tc.trim() + '\':\n' + notes + 'Markup with issue:\n' + snippet + '\n---------------\n';
            row += notes + '<br><button class="showSnippet" type="button" aria-label="Show markup snippet" aria-expanded="false"><code>&lt;/&gt;</code></button><div class="snippet" hidden><label for="snip' + i + '">Markup snippet</label><textarea id="snip' + i + '" aria-label="Markup snippet for this node">' + snippet + '</textarea><button type="button" class="decrapulate" aria-label="De-crapulate this markup snippet">De-crapulate</button></div></td>';
            row += '<td><button data-title-ref="' + i + '" class="highlightButton" type="button" aria-pressed="false" aria-label="Highlight this issue on the page visually">Show</button></td>';
            row += '</tr>';
            i++;
            if (warn || err) {
                issueCount++;
        ...
        if (identCount > 0) {
            s += '<input type="checkbox" id="showPotentialProblemsOnly"><label for="showPotentialProblemsOnly">Show only elements with definite title issues(' + issueCount + ').</label><br>'; /*s+=identCount + ' elements with identical titles were found.';*/
        }
        s += ' <button class="highlightButtonAll" type="button" aria-pressed="false">Highlight all elements with `title` on page</button>';
        s += '<table border="1" cellpadding="5"><caption>All things with title attributes on this page</caption><thead><tr valign=top><th scope="col">On-screen text</th><th>Title text</th><th>Interactive?</th><th scope="col">Notes</th><th>Highlight on the page</th></tr></thead><tbody>' + row + '</tbody></table>';
